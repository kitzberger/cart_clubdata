...
namespace Extcode\CartClubdata\Domain\Repository;
/**
...
class PauseRepository extends \TYPO3\CMS\Extbase\Persistence\Repository
{
    ...
    public function findPause($date)
    {
        ...
        $query->setOrderings(array('fromdate' => \TYPO3\CMS\Extbase\Persistence\QueryInterface::ORDER_ASCENDING));
        $and_constraints = array();
        $and_constraints[]=$query->greaterThanOrEqual('todate',$date);
        $and_constraints[]=$query->lessThan('fromdate',$date);
        if ($and_constraints) $query->matching($query->logicalAnd($and_constraints));
