...
namespace Extcode\CartClubdata\Service;
...
class PdfServiceTickets extends \Extcode\CartPdf\Service\PdfService
{
    ...
    protected function renderPdf($pdfType, $orderItem)
    {
        ...
        if (!$this->pdfSettings['footer']) {
            ...
            } else $this->pdf->setAutoPageBreak(true, PDF_MARGIN_BOTTOM);
        }
       if ($pdfType == 'delivery') {
           $this->pdf->setAutoPageBreak(false, 0);
           $this->pdf->AddPage('L',array(210,99));
       }
    ...
    protected function renderTicketBody($pdfType, $orderItem)
    {
        ...
        foreach ($orderItem->getProducts() as $product) {
            ...
            for ($i = 0; $i < $product->getCount(); $i++) {
                ...
                if ($i) $code=$this->addEanCheck($base+=1);
                ...
                $style = array(
                    ...
                    foreach ($this->pdfSettings['ticket'] as $partName => $partConfig) {
                        if ($anz && !$mod) $partConfig['positionY'] = $anz*80;
